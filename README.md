...
冒烟脚本会使用临时数据目录和确定性的本地 embedding 替身，不需要真实模型 API Key；它会覆盖登录、文档检索、系统健康、备份清单、索引修复、运维事件、备份校验、恢复预检、真实恢复拒绝闸门和临时恢复演练。`--frontend` 模式要求已执行过 `cd frontend && npm ci`。
...
服务启动后还会按 `ops.auto_backup_*` 配置执行后台备份维护：默认每 24 小时备份 `config.yaml`、`data/`、`workspace/`，并保留最近 14 个本地归档；最近一次自动维护结果会显示在管理员系统状态页，管理员也可以在该页面手动触发一次即时备份。后台备份不包含主机上的 `.env`；升级、迁移或外部归档前仍建议使用上面的 CLI 备份命令做一次完整校验。
...
| `GET` | `/api/system/events` | 管理员查看最近运维事件 |
