...
服务启动后还会按 `ops.auto_backup_*` 配置执行后台备份维护：默认每 24 小时备份 `config.yaml`、`data/`、`workspace/`，并保留最近 14 个本地归档；最近一次自动维护结果会显示在管理员系统状态页。后台备份不包含主机上的 `.env`；升级、迁移或外部归档前仍建议使用上面的 CLI 备份命令做一次完整校验。
