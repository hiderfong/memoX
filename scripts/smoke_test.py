...
def run_operational_checks(
    ...
    *,
    ...
    diagnostics = checks.record(
        check_name("export diagnostics"),
        client.get(f"{base_url}/api/system/diagnostics/export", headers=headers),
        {200},
    )
    if diagnostics.headers.get("content-type") != "application/zip" or not diagnostics.content.startswith(b"PK"):
        raise RuntimeError("diagnostic export did not return a zip file")
